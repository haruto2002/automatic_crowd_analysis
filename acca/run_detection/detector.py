...
class Detector:
    ...
    def run(self, video_path, save_dir):
        ...
        #     self.dtype,
        # )
        self.run_inference_multiple_frames(
            video_path,
            save_dir,
            self.detector,
            self.transformer,
    ...
    def transform_multiple_frames(self, frames, transformer, dtype):
        frames = (
            frames.div(255).permute(0, 3, 1, 2).to(dtype)
        )  # (B, H, W, 3) -> (B, 3, H, W)
        transformed = transformer(frames)
        return transformed
    ...
    def run_inference_multiple_frames(
        self,
        video_path,
        save_dir,
        detector,
        ...
        sequence_length = (
            1  # sequence_lengthで割り切れない場合は、最後のフレームが捨てられる
        )
        pipeline = self.video_pipe(
            batch_size=1,
            num_threads=8,
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=dtype):
            for i in tqdm(
                ...
                frames = sequence[0]
                transformed_frames = self.transform_multiple_frames(
                    frames, transformer, dtype
                )
                outputs = detector(transformed_frames)
                outputs_scores = (
                    torch.nn.functional.softmax(outputs["pred_logits"], -1)[:, :, 1]
                    .detach()
                    .cpu()
